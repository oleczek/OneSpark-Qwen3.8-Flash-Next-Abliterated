...
class Qwen4ExpNGramEmbedding(nn.Module):
    ...
    def _embed_ngram_ids(
        ...
        if getattr(self.ngram_embedding, "hashk_mode", False):
            ...
            if _os.environ.get("SGLANG_QSA_SHIM_DEBUG") == "1":
                logger.warning(
                    "PLE gather: ids_shape=%s ids_max=%d rows=%d emb_nan=%s emb_amax=%.3f",
                    tuple(lookup_ids.shape), int(lookup_ids.max().item()),
                    self.ngram_embedding.weight_packed.shape[0],
                    bool(torch.isnan(embeddings).any()),
                    embeddings.float().abs().amax().item(),
                )
        else:
...
class Qwen4ExpLinearDecoderLayer(
    ...
    def forward(
        ...
        **kwargs,
        ...
        _dbg = (
            _os.environ.get("SGLANG_QSA_SHIM_DEBUG") == "1"
            and forward_batch is not None
            and forward_batch.forward_mode.is_decode()
            and getattr(self, "layer_id", 99) <= 4
        )
        ...
        def _st(tag, t):
            if _dbg and t is not None:
                logger.warning(
                    "L%s %s: nan=%s amax=%.2f",
                    getattr(self, "layer_id", "?"), tag,
                    bool(torch.isnan(t).any()), t.float().abs().amax().item(),
                )

        _st("in", hidden_states)
